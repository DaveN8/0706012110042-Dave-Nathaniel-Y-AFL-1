...
var cart: [(namaToko: String, namaFood: String, jumlahFood: Int)] = []
var total: Int = 0
        ...
        if (Int(userInput) ?? 0 > 0){
            ...
            for (index, check) in cart.enumerated() {
                    if check.1 == menuhuy1[inputan-1].0{
                        let temp: Int = check.jumlahFood + jumlah
...
// error handling untuk cart biar ga tampil semua
// kurang error handling untuk inputan checkout harus int
